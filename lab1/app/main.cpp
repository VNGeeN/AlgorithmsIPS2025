...
private:
    ...
    void processTerminalInput()
    {
        ...
        while (true)
        {
            ...
            try
            {
                ...
                int postfixSize = 0;
                parser.toPostfix(input, postfix, postfixSize);
                double result = parser.evaluatePostfix(postfix, postfixSize);
                ...
                delete[] postfix; // Важно: освобождаем память
            }
    ...
    void processTestFile(const std::string &filename)
    {
        ...
        while (std::getline(file, line))
        {
            ...
            try
            {
                // ИСПРАВЛЕНИЕ: используем новый интерфейс toPostfix
                std::string *postfix = nullptr;
                int postfixSize = 0;
                parser.toPostfix(line, postfix, postfixSize);
